...
module Mercadopago
  describe ProcessNotification do
    ...
    describe 'with valid operation_info' do
      ...
      before do
        fake_client = instance_double('Mercadopago::Client')
        fake_payment_method = instance_double('Spree::PaymentMethod::Mercadopago')
        allow(fake_payment_method).to receive(:provider).and_return(fake_client)
      ...
      describe '#process!' do
        it 'completes payment for approved payment' do
          described_class.new(notification).process!
          payment.reload
